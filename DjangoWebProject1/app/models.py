...
class Comment(models.Model):
    ...
    date = models.DateTimeField(default=now, editable=False, verbose_name = "Дата комментария")
    author = models.ForeignKey(User, null=True, blank=True, on_delete = models.SET_NULL, verbose_name = "Автор")
    post = models.ForeignKey('Blog', on_delete=models.CASCADE, verbose_name = "Статья комментария")

    # Название , которое используется для представления отдельных записей в административном разделе
    def __str__(self):
...
admin.site.register(Blog) #редактирование данных в админ разделе